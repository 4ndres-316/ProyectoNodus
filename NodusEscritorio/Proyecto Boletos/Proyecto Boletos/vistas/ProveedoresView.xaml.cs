...
namespace Proyecto_Boletos.vistas
{
    public partial class ProveedoresView : UserControl
    {
        ...
        private readonly List<FilaServicioProveedor> _filasServicio = new List<FilaServicioProveedor>();
        ...
        public ProveedoresView(int idRol)
        {
            ...
            txtBuscarCatalogo.TextChanged += (s, e) => ConstruirCatalogo(txtBuscarCatalogo.Text?.Trim() ?? "");
            txtBuscarResumen.TextChanged  += (s, e) => ConstruirResumen(txtBuscarResumen.Text?.Trim() ?? "");
        }
        ...
        private async Task CargarDatos()
        {
            ...
            catch { _detalles = new List<DetalleProveedorServicio>(); }
            ...
            try
            {
                ...
                foreach (var p in _proveedores)
                {
                    ...
                    var ps     = _proveedorServicios?.Where(x => x.IdProveedor == p.NitProveedor).ToList();
                    var partes = ps?.Select(x =>
                    {
                        var srv = _servicios?.Find(s => s.IdServicio == x.IdServicio);
                        return srv?.NombreServicio ?? x.IdServicio.ToString();
                    }).ToList();

                    p.ServiciosConPrecio = partes?.Count > 0 ? string.Join(" | ", partes) : "Sin servicios";
                }

                dgProveedores.ItemsSource = _proveedores;

                txtTotalProveedores.Text     = _proveedores.Count.ToString();
                txtProveedoresActivos.Text   = _proveedores.Count(p => p.EstadoProveedor == "Activo").ToString();
                txtProveedoresInactivos.Text = _proveedores.Count(p => p.EstadoProveedor == "Inactivo").ToString();
            }
            catch (Exception ex)
            {
        ...
        private void ConstruirCatalogo(string filtro)
        {
            ...
            if (_categorias == null || _servicios == null || _proveedores == null) return;
            ...
            foreach (var cat in _categorias.OrderBy(c => c.NombreCategoria))
            {
                ...
                if (serviciosCat.Count == 0) continue;
                ...
                var gruposProveedor = (_proveedorServicios ?? new List<ProveedorServicio>())
                    ...
                    .Where(x => x.Prov != null && x.Prov.EstadoProveedor == "Activo" && x.Items.Count > 0)
                    .OrderBy(x => x.Prov.NombreComercial)
                    .ToList();

                if (gruposProveedor.Count == 0) continue;
                ...
                var wrap = new WrapPanel { Orientation = Orientation.Horizontal, Margin = new Thickness(0, 0, 0, 6) };
                ...
                foreach (var grupo in gruposProveedor)
                {
                    ...
                    foreach (var srvItem in grupo.Items)
                    {
                        ...
                        var dets = (_detalles ?? new List<DetalleProveedorServicio>())
                            .Where(d => d.IdProveedor == grupo.Prov.NitProveedor
                        ...
                        if (dets.Count > 0)
                        {
                            foreach (var det in dets)
                            {
                                ...
                                fD.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
                                fD.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });
            ...
            if (panelCatalogo.Children.Count == 0)
                panelCatalogo.Children.Add(new TextBlock
                {
        ...
        private void ConstruirResumen(string filtro)
        {
            ...
            foreach (var p in lista.OrderBy(x => x.NombreComercial))
            {
                var serviciosDelProv = _proveedorServicios
                    ...
                    .Select(ps =>
                    {
                        ...
                        return new { Nombre = srv?.NombreServicio ?? "-", ps.IdServicio };
                    })
                    .OrderBy(x => x.Nombre)
                    .ToList();
                ...
                cabeceraGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
                cabeceraGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });
                ...
                var estadoBadge = new Border
                {
                    ...
                    CornerRadius = new CornerRadius(10),
                    Background   = p.EstadoProveedor == "Activo"
                    ...
                    Child = new TextBlock
                    {
                        ...
                        FontSize   = 11,
                        Foreground = p.EstadoProveedor == "Activo"
                ...
                else
                {
                    foreach (var srvInfo in serviciosDelProv)
                    {
                        ...
                        var dets = (_detalles ?? new List<DetalleProveedorServicio>())
                            .Where(d => d.IdProveedor == p.NitProveedor && d.IdServicio == srvInfo.IdServicio)
                            .OrderBy(d => d.NombreItem)
                            .ToList();
                ...
                panelResumen.Children.Add(new Border
                {
                    Child           = cardStack,
                    Width           = 280,
                    Margin          = new Thickness(5),
            ...
            if (panelResumen.Children.Count == 0)
                panelResumen.Children.Add(new TextBlock
                {
                    Text = "No se encontraron proveedores.",
                    FontStyle = FontStyles.Italic,
                    Foreground = Brushes.Gray,
        ...
        private void AgregarFilaServicio(int? idServicio = null,
        ...
        private async void btnGuardar_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("El nombre comercial es obligatorio.", "Validación",
                    MessageBoxButton.OK, MessageBoxImage.Warning); return;
            }
            if (string.IsNullOrWhiteSpace(txtTelefonoComercial.Text))
            {
                MessageBox.Show("El teléfono comercial es obligatorio.", "Validación",
                    MessageBoxButton.OK, MessageBoxImage.Warning); return;
            }
            ...
            foreach (var fila in _filasServicio)
            {
                ...
                {
                    MessageBox.Show("Hay servicios sin seleccionar. Complétalos o elimínalos.",
                        "Validación", MessageBoxButton.OK, MessageBoxImage.Warning); return;
                }
            }
            ...
            {
                MessageBox.Show("Hay servicios repetidos. Cada servicio solo puede aparecer una vez.",
                    "Validación", MessageBoxButton.OK, MessageBoxImage.Warning); return;
            }
            ...
            try
            {
                if (_modoEdicion)
                {
                    ...
                    _proveedorSeleccionado.EstadoProveedor   =
                        ((ComboBoxItem)cmbEstadoProveedor.SelectedItem).Content.ToString();
                    await ConexionDB.Client.From<Proveedor>().Update(_proveedorSeleccionado);
                }
                else
                {
                    await ConexionDB.Client.From<Proveedor>().Insert(new Proveedor
                    {
                        NitProveedor      = nit,
                ...
                await ConexionDB.Client.From<ProveedorServicio>()
                    .Filter("id_proveedor", Supabase.Postgrest.Constants.Operator.Equals, nit.ToString())
                    .Delete();

                foreach (var fila in _filasServicio)
                {
                    if (fila.IdServicioSeleccionado == null) continue;
                    await ConexionDB.Client.From<ProveedorServicio>().Insert(new ProveedorServicio
                    {
                        IdProveedor = nit,
                ...
                await ConexionDB.Client.From<DetalleProveedorServicio>()
                    .Filter("id_proveedor", Supabase.Postgrest.Constants.Operator.Equals, nit.ToString())
                    .Delete();
                ...
                foreach (var fila in _filasServicio)
                {
                    ...
                    {
                        if (string.IsNullOrWhiteSpace(item.Nombre)) continue;
                        await ConexionDB.Client.From<DetalleProveedorServicio>().Insert(
        ...
        private void btnEditar_Click(object sender, RoutedEventArgs e)
        {
            ...
            foreach (ComboBoxItem item in cmbEstadoProveedor.Items)
                ...
                { cmbEstadoProveedor.SelectedItem = item; break; }
        ...
        private async void btnDeshabilitar_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("El proveedor ya está inactivo.", "Aviso",
                    MessageBoxButton.OK, MessageBoxImage.Information); return;
            }

            if (MessageBox.Show($"¿Deshabilitar a '{_proveedorSeleccionado.NombreComercial}'?",
            ...
            try
            {
                ...
                ConstruirCatalogo(txtBuscarCatalogo.Text?.Trim() ?? "");
                ConstruirResumen(txtBuscarResumen.Text?.Trim() ?? "");
                MessageBox.Show("Proveedor deshabilitado.", "Éxito",
                    MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
    ...
    public class FilaServicioProveedor
    {
        ...
        public FilaServicioProveedor(List<Servicio> servicios, int? idServicio = null,
            List<(string Nombre, long Precio)> itemsExistentes = null)
        {
            ...
            _cmbServicio.SelectionChanged += (s, e) =>
                IdServicioSeleccionado = _cmbServicio.SelectedValue != null
                    ? (int?)_cmbServicio.SelectedValue
                    : null;
            ...
            filaTop.Children.Add(new TextBlock
            {
                Text              = "Servicio:",
    ...
    public class FilaItemServicio
    {
        ...
        {
            get { long.TryParse(_txtPrecio.Text, out long p); return p; }
        }
